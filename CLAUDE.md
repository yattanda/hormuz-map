...
- **日付を書き込む前に、その都度コマンドで現在日時を実測する。**
  ...
  （`.claude/settings.local.json` の UserPromptSubmit フックが毎ターン `[現在日時]` を注入する。
   注入値が無い場合は `date` を実行して確認する）
...
- canonical / og:url など移行で変わる URL は**ハードコードせず `{{base_url}}` 変数のまま残す**（一括展開は移行スクリプトが担う。ドメイン確定前にURLを固定すると移行漏れが出るため）
