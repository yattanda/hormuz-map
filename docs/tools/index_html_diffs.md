...
## [追加修正] シナリオセクション見出しの基準日修正（2/28統一に伴う修正）

> 封鎖日カウンター基準をDay0=2/28に統一する決定（本日のユーザー確定事項）に伴い、
> シナリオセクション見出しに残っていた「4/15 封鎖発動後」という古い基準表記を「2/28 開戦後」に修正します。
> これは日次更新セクション（S01〜S11）とは別の、静的見出し部分の一回限りの修正です。

<!-- APPLY:START -->
<!-- OLD:START -->
<div class="sec-title">📊 4つのシナリオ（4/15 封鎖発動後・今後の展開）<span class="label-scenario" style="margin-left:8px;font-size:0.7rem;">シナリオ分析</span></div>
<!-- OLD:END -->
<!-- NEW:START -->
...
サイト内を検索したところ、以下の「4/13」表記は**封鎖日カウンターの基準日とは無関係な、個別の史実日付への言及**であり、修正不要と判断しました（そのまま残してください）：

- MAP凡例の「最終更新：2026/04/13 07:00 JST」（地図データの最終更新時刻）
- 更新ログ末尾の出典リンク③④⑤⑧内の「（4/13）」（米国のイラン港湾封鎖発表・報道日の注記）
- 旧ティッカーのコメントアウト（4/13 7:00版、既にコメントアウト済みで非表示）

これらは「2重封鎖（米国による対イラン港湾封鎖）が4/13に始まった」という事実そのものを指しており、基準日変更の影響を受けません。

---

## [追加修正2] シナリオ確率更新補足ブロックの見落とし修正（S06の再修正）

> ご指摘の通り、前回のS06修正は「各シナリオ確率は...」の1行のみを直しており、
> その直上にある「📊 2026年7月30日 10:08 JST 更新」の日付見出しと、
> 続く要約文（A↓B→C↑D↑の説明、7/28〜30時点の内容のまま）が未修正でした。
> ブロック全体を8/1版に更新します。
...
✓ シナリオ確率更新補足ブロック修正完了（S06完全版）

---

## [追加修正3] 🚢 全ルート現況サマリー（S08.5・作業漏れの修正）

> **これは作業漏れでした。** daily-site-update SKILLの手順「8.5. 全ルート現況サマリー 更新（S08完了後・30秒カラムの直前）」を
> 今回のdiffs生成時に見落としており、当該セクションが7/30のまま取り残されていました。
> ご指摘のとおり、意図的なものではなく単純なミスです。以下で修正します。
...
✓ 全ルート現況サマリー修正完了（S08.5）

---

## ✅ 追加セルフチェック（今回の3件の追加修正について）

```
[x] シナリオ見出し「4/15封鎖発動後」→「2/28開戦後」に修正 ✓
[x] シナリオ確率補足ブロック（日付見出し＋A↓B→C↑D→要約文）を8/1版に更新 ✓
[x] 全ルート現況サマリー（S08.5・作業漏れ）を8/1版に更新 ✓
```

### 今回判明した反省点（次回以降の運用改善）

- **全ルート現況サマリー（S08.5）の見落とし**：`daily-site-update` SKILLの作業順序リストにある項目だが、`diffs-generation-rules.md`側のスケルトンには明示的に含まれていなかったため見落とした可能性がある。次回セッションで、diffs-generation-rules.md側のセクション対応表にもS08.5を追記することを推奨する。
- **シナリオセクション内の「日付が2箇所に分かれている」構造**：sec-title直下の「📊 更新」見出しと、確率バナー末尾の「各シナリオ確率は...時点」の2箇所に同趣旨の日付があり、片方だけ更新して見落とすリスクがある。次回セルフチェック項目に「シナリオセクション内の日付表記は何箇所あるか列挙し、全て更新したか」を追加することを推奨する。
