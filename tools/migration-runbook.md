...
**逆方向も守る。移行を始める前に作ったクラウドセッションの日次更新を、移行後に main へマージしない。**

- 日次更新が書き換える JSON-LD の `dateModified` と、手順5で置換する `url` は
  `docs/index.html` の同じ塊の中で数行しか離れていない。移行前の状態から作った変更を
  移行後にマージすると、ここで競合しやすい
- 移行中（手順3〜6）はスマホから新しい日次更新を始めない。
  移行の push が済んでから**新しいクラウドセッションを始める**（最新の main から作業が始まる）
- 移行前に始めてしまったセッションが残っていたら、マージせず閉じ、新しいセッションでやり直す
- 移行作業そのものはクラウドセッションで行わない。GitHub の Pages 設定・`docs/CNAME` の pull・
  スクリプト実行を伴うため PC で行う
