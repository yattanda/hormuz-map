...
| `@chokepointlab` の本運用開始 | **未** | 9/17 までに X 上で最低条件を満たす（手順8「前提」）。未達でも移行は進めてよい。サイトのリンク差し替え（8-2）だけ延期する |
...
## 8. X（旧Twitter）の切り替え（§11-12(C)）

2026-09-06 の決定（案ア）：ドメイン移行と同時に、発信アカウントを `@chokepointlab` へ切り替える。
「サイトの引っ越し」と「発信アカウントの切り替え」を一度に起こし、読者への説明を1回で済ませるため。

### 前提：本運用開始の最低条件（9/17 までに X 上で済ませる）

「本運用開始」は記録上定義が無かったため、2026-09-13 に次を最低条件と定めた。
X 上の作業でサイトには触れないので、**凍結期間中に行ってよい**。
...
**条件を満たしていない場合**：8-1 と 8-2 は延期し、移行だけ完了させる。
メタ情報は既に `@chokepointlab` を指しているので、延期しても不整合は増えない。

### 8-1. `@chokepointlab` のウェブサイト欄

- **手順6の本番検証が通り、HTTPS 証明書が発行された後**に、ウェブサイト欄を `https://chokepointlab.com` にする
...
**決める期限**：当日に告知を出すなら 9/17 まで。決まっていなければ、当日は何もしない（併存の状態のまま）でよい。
